...
async function run() {
  ...
  check('ground segment: elevation reported per station',
    await page.$$eval('.gs-el', (n) => n.every((e) => /EL -?\d/.test(e.textContent))));
  ...
  // --- audio ---------------------------------------------------------------
  check('audio: engine constructed', await page.evaluate(() => Boolean(window.__audio)));
  check('audio: no media files requested — everything synthesised',
    await page.evaluate(() => performance.getEntriesByType('resource')
      .every((r) => !/\.(mp3|ogg|wav|m4a|aac|flac)(\?|$)/i.test(r.name))));
  check('audio: button reports a real state',
    /AUDIO (ON|ARMED|OFF)/.test(await text(page, '#btn-audio')));
