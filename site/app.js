...
async function main() {
  ...
  // Browsers will not start audio without a gesture, so the button reads
  // ARMED until one arrives rather than claiming to be on.
  ...
  boot('audio engine — synthesised, no assets');
  ...
  setTimeout(() => {
    ...
    tour.maybeAutoStart({ delayMs: 3000 });
  }, 520);
}
