...
export function updateTopbar(state) {
  ...
  const el = $('v-orbit');
  el.textContent = `#${state.clock.orbit_number} · ${f(c, 1)}× real time`;
...
export function updatePower(state) {
  ...
  $('power-kv').innerHTML = `
    ...
    <dd class="${b.margin_to_floor_pct < 12 ? 'warn' : ''}"
        title="Percentage points of state of charge above the ${b.flight_rule_floor_pct} % flight-rule floor.">${f(b.margin_to_floor_pct, 1)} % pts</dd>`;
