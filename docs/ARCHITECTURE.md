...
Synthesised at runtime with the Web Audio API — oscillators, filtered noise and
envelopes. **No audio files ship**, which is what keeps the console a genuinely
self-contained static site: nothing to bundle, nothing to fetch, nothing to 404
on a Pages subpath. The headless check asserts that the page requests no media
files at all.

The design reads the mission rather than decorating it:

- The drone is pitched and filtered by operating mode, so the room can hear the
  spacecraft come out of eclipse.
- The tension sweep is driven by `clock.time_compression`, normalised against
  `clock.compression_bounds`. It tightens as the console slows into the
  overpass because it is reading the same number the timeline curve draws, not
  running on a timer of its own.
- AOS and LOS chirps are emitted by the ground-segment panel's own transition
  detection, so the audio layer never re-derives state.
- Alerts sound on *change* of the highest-severity code, not every tick.

Browsers will not start audio without a user gesture, so the context is built
lazily and the button reads `AUDIO ARMED` until one arrives — the console does
not claim to be making a sound it is not making. `prefers-reduced-motion`
