"""
Mission configuration constants for AL-FALAJ SAT-1.

Everything the mission model needs to be defensible in a technical Q&A lives
here, with a source note attached to each number.

This module and `site/engine/config.js` must agree exactly; the browser port
is what GitHub Pages runs and `tools/parity-check.mjs` asserts they match.
"""
import math
...
# ---------------------------------------------------------------------------
ORBIT_PERIOD_S = SPACECRAFT["orbit"]["period_min"] * 60.0    # 5694 s
LOOP_DURATION_S = 90.0

# Where the spacecraft is, in orbit seconds, at each point in its orbit.
#
# The loop covers exactly one orbital period, so the ground track closes: the
# sub-satellite point at loop t = 90 s is the point at t = 0 s.
#
...
# which the recorder is dumped before the spacecraft re-enters the umbra. The
# pass is deliberately *not* at the loop boundary: a climax that lands on the
# wrap has nowhere to resolve.
ORBIT_SEGMENTS = [
    ...
    {"mode": "SUN_FACING", "orbit_start_s": 5245.0,
     "orbit_end_s": ORBIT_PERIOD_S, "egress": True},
]

PASS_MID_ORBIT_S = 5172.5
...
# natural periodic Gaussian: smooth, strictly positive, and identical at x = 0
# and x = 1, so the loop has no seam in its *rate* either — the wrap is
...
def warp_density(x: float) -> float:
    """Loop seconds per orbit second at orbit fraction ``x``."""
    th = 2.0 * math.pi * (x - WARP["phase_pass"])
    return 1.0 + WARP["amplitude"] * math.exp(
        WARP["concentration"] * (math.cos(th) - 1.0))
...
def _build_warp_table():
    ...
    Built once, by trapezoid, in the same order as the JavaScript port so the
    ...
    for k in range(1, n + 1):
        x0 = (k - 1) / n
        x1 = k / n
        acc += (0.5 * (warp_density(x0) + warp_density(x1))) / n
        cum.append(acc)
    total = cum[n]
    return total, [LOOP_DURATION_S * c / total for c in cum]


WARP_TOTAL, WARP_LOOP_AT = _build_warp_table()


def loop_time_at_orbit(orbit_s: float) -> float:
    """Loop seconds elapsed at a given orbit time."""
    n = WARP["samples"]
    x = (max(0.0, min(ORBIT_PERIOD_S, orbit_s)) / ORBIT_PERIOD_S) * n
    k = min(n - 1, int(math.floor(x)))
    f = x - k
    a = WARP_LOOP_AT[k]
    return a + f * (WARP_LOOP_AT[k + 1] - a)
...
def _timeline_segments():
    ...
    """
    out = []
    for s in ORBIT_SEGMENTS:
        info = MODE_INFO[s["mode"]]
        egress = bool(s.get("egress"))
        out.append({
            "mode": s["mode"],
            "label": "SUN FACING · EGRESS" if egress else info["label"],
            "banner": ("PASS COMPLETE · DUMPING RECORDER" if egress
                       else info["banner"]),
            "description": (
                "Imaging complete. Still sunlit, arrays recharging, "
                "solid-state recorder dumping to the ground segment before "
                "the next eclipse." if egress else info["description"]),
            "payload_state": info["payload_state"],
            "egress": egress,
            "orbit_start_s": s["orbit_start_s"],
            "orbit_end_s": s["orbit_end_s"],
            "start_s": round(loop_time_at_orbit(s["orbit_start_s"]), 4),
            "end_s": (LOOP_DURATION_S if s["orbit_end_s"] >= ORBIT_PERIOD_S
                      else round(loop_time_at_orbit(s["orbit_end_s"]), 4)),
        })
    return out
...
MODES = ["ECLIPSE", "SUN_FACING", "ACTIVE"]
...
GROUND_STATIONS = [
    {"id": "DXB_GS", "name": "Dubai Primary", "short": "DUBAI",
     "lat": 25.2048, "lon": 55.2708, "band": "X", "rate_mbps": 120,
     "role": "Mission home station — acquires before the pass and holds "
             "through it."},
    {"id": "INU_GS", "name": "Inuvik, Canada", "short": "INUVIK",
     "lat": 68.3190, "lon": -133.5490, "band": "X", "rate_mbps": 310,
     "role": "High-latitude dump on the sunlit arc."},
    {"id": "TRL_GS", "name": "Troll, Antarctica", "short": "TROLL",
     "lat": -72.0117, "lon": 2.5350, "band": "X", "rate_mbps": 280,
     "role": "Polar dump during eclipse, before the spacecraft comes back "
             "into sun."},
]
...
# Provenance
# ---------------------------------------------------------------------------
# Where every number on the console comes from. A dashboard that cannot say
# what produced a figure is decoration. Each panel carries one of these lines
# verbatim.
PROVENANCE = {
    "orbit": "Keplerian propagation · 520 km SSO, i=97.49° · Earth rotation "
             "held fixed for loop closure",
    "power": "EPS energy balance · 0.10 m² GaAs @ 29.8% EOL, 78 Wh Li-ion, "
             "CC/CV taper",
    "spectro": "HYPER-A L1B → Rrs(λ) · 96 bands, 400–900 nm @ 5.2 nm",
    "science": "MCI / NDCI / FLH retrieval · MERIS-OLCI band algebra on Rrs(λ)",
    "desal": "Intake scheduler · bloom risk × stratification × tide, "
             "MERIS-OLCI Chl-a forcing",
    "comms": "Link budget from slant range & elevation · X-band, 10° mask",
    "impact": "Capacity at risk = plant MIGD × fouling factor · 4546.09 m³ "
              "per MIGD",
    "downlink": "CCSDS Space Packet telemetry · every field mirrored from "
                "this frame",
    "map": "Natural Earth coastlines (public domain) · swath from payload "
           "FOV & attitude",
}
